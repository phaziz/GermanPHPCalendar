...
<html class="no-js" lang="en">
        ...
        <style>
        	table{margin: 25px auto;border: 1px solid #000;background:#fff;width:100%;}
        	th{color:#000 !important;background:#fff;border:1px solid #fff}
			td{height: 100px; width: 100px;border: 1px solid #fff;}
			td{background: #000;color:#fff !important;}
			td.empty{background: #000;}
			td.active{background: #fff;color:#000 !important;}
			.feiertag, .has-tip{cursor:help;color:#fff !important;text-decoration:none;border:none;}
    ...
    <body>
		...
		<div class="row margin-top-25">
			<div class="small-12 large-12 columns">
				<form name="datr" id="datr" action="index.php" method="get" enctype="text/plain">
					<div class="row collapse">
						...
						<div class="small-2 large-2 columns">
							<a href="" id="submitr" name="submitr" role="button" aria-label="submit form" class="button success postfix">Zeigen</a>
						</div>
					</div>
				</form>
			</div>	
		...
		<div class="row">
			...
			<?php
				...
				while($DAY_NUM <= $DAYS_IN_MONTH)
				{
					...
					{
						echo '<td scope="row" class="text-center active">' . $DAY_NUM . '<br><small>' . $THE_DAY . '.' . $ACT_MONTH_NUM . '.' . $YEAR . '</small></td>';	
					}
					else
					{
						$COMPLETE_DAY_DATE = $THE_DAY . '.' . $ACT_MONTH_NUM . '.' . $YEAR;

						echo '<td scope="row" class="text-center">' . $DAY_NUM . '<br>';
						echo '<small>' . $COMPLETE_DAY_DATE . '</small>';

						$FEIERTAG = checkFeiertag($COMPLETE_DAY_DATE);

						if($FEIERTAG != FALSE)
						{
							echo '<br><span role="tooltip" data-options="hover_delay: 50;" data-tooltip aria-haspopup="true" class="feiertag has-tip" title="' . $FEIERTAG . '">#</span>';
						}
